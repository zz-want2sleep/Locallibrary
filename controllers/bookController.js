...
exports.book_create_post = [
    ...
    body('author', '作者不能为空,你需要先增添作者.').trim().isLength({
        min: 1
    }),
    body('summary', '概要不能为空,你需要先增添概要.').trim().isLength({
        min: 1
    }),
    body('isbn', 'ISBN 不能为空,你需要先增添ISBN.').trim().isLength({
        min: 1
    }),
    ...
    (req, res, next) => {
        // console.log(req.body.genre.toString())
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                for (let i = 0; i < results.genres.length; i++) {
                    if (book.genre.toString().indexOf(results.genres[i]._id) > -1) {
                        results.genres[i].checked = 'true';
                    }
                }

                console.log(typeof results.authors[0]._id)
                res.render('book_form', {
                    title: '创建藏书',
                    authors: results.authors,
                    genres: results.genres,
