...
// 由 POST 处理作者创建操作
exports.author_create_post = [ // Validate fields.
    body('first_name').isLength({
        min: 1
    }).trim().withMessage('必须指定名字！')
    .isAlphanumeric().withMessage('名字有非字母数字字符！'),
    body('family_name').isLength({
        min: 1
    }).trim().withMessage('Family name must be specified.')
    .isAlphanumeric().withMessage('姓氏有非字母数字字符！'),
    body('date_of_birth', '无效的出生日期！').optional({
        checkFalsy: true
    }).isISO8601(),

    body('date_of_death', '无效的死亡日期！').optional({
        checkFalsy: true
    }).isISO8601(),
    //optional可是我们仅在输入字段时运行后续验证
    // Sanitize fields.
